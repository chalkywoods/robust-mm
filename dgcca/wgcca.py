...
class WeightedGCCA:
  ...
  def corr(self, views, K=None, scaleBySv=False):
    embedding = self.apply(views, K, scaleBySv)
    if len(embedding) > len(self.G):
      return self.offset_corr(embedding, self.G)
    else:
      return self.offset_corr(self.G, embedding)
  
  def offset_corr(self, longer, shorter):
    windows = len(longer) // len(shorter)
    window_length = len(shorter)
    corr_sum = 0
    for i in range(windows):
      corr_sum += np.corrcoef(longer[i:i+window_length], shorter, rowvar=False)[0,1]
    return corr_sum/windows
